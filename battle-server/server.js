...
function canReceiveLivePeerEvent(playerSession) {
  ...
  if (playerSession.moveSeen) return true;
  if (playerSession.dead) return true;
  return !playerSession.spawned && ALLOW_PRESPAWN_SPECTATOR_LIVE;
}
...
async function handleOperation(port, socket, rinfo, session, parsed, channel = 0) {
  ...
  if (eventCode === 93) {
    ...
    if (response) broadcastReliableToRoom(session, response, channel, "item-pick", { requireLiveReady: true });
    return response ? [response] : [];
  }
  ...
  if (eventCode === 98) {
    ...
    if (response) broadcastReliableToRoom(session, response, channel, "weapon-change", { requireLiveReady: true });
    return response ? [response] : [];
  }
  if (eventCode === 77) {
