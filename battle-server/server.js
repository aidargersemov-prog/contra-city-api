...
const BUILD_ID = "battle-server-2026-07-10-combat-runtime-contract-v248";
...
const APPLY_PROFILE_RUNTIME_BONUSES = process.env.APPLY_PROFILE_RUNTIME_BONUSES === "1";
...
function gameplayModifiersForProfile(profile = null) {
  ...
  if (!APPLY_PROFILE_RUNTIME_BONUSES) return modifiers;
...
function impactDamageReductionForTarget(targetSession, impactType) {
  ...
  const enhancerReduction = APPLY_PROFILE_RUNTIME_BONUSES && enhancerType && hasSelectedEnhancer(targetSession.loadedProfile, enhancerType) ? 50 : 0;
